...
template <typename T, typename U>
auto operator << (std::ostream & out, std::unordered_map<T, U> const & v)
    -> std::ostream &
{
    std::cout << "{";
...
auto get_value(std::unordered_map<int, int> & counter
              ,Iterator             start
              ,Iterator             to_remove)
    -> int
{
    auto iter = counter.find(*start);
    if( iter != counter.end() )
    {
        ++(iter->second);
    }
    ...
    {
        counter.emplace(*start, 1);
    }
    auto prev = counter.find(*to_remove);
    --(prev->second);
    if( prev->second == 0 )
        counter.erase(prev);
